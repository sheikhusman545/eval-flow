...
function wireFeReferenceClipsModal() {
  var overlay = document.getElementById("clipsOverlay");
  if (overlay && overlay.getAttribute("data-fe-rc-wired") === "1") return;
  ...
  var hasAnyTrigger = !!btnOpen || !!document.querySelector(".fe-stroke-add-clip");
  if (
    !overlay ||
    !clipList ||
    !tabsWrap ||
  ...
  document.querySelectorAll(".fe-stroke-add-clip").forEach(function (btn) {
    btn.addEventListener("click", function () {
      var card = btn.closest(".app-eval-stroke-card");
      _activeStrokeRow = card ? card.querySelector(".fe-stroke-clips-row") : null;
      hideAddView();
      overlay.classList.add("is-open");
      showTab(activeTab);
      requestAnimationFrame(function () {
  ...
  /* Filter toggle â€” shows the right panel based on active tab */
  ...
  function removeReferenceClipById(idStr) {
    var allRows = [selectedClipsRow, previewClipsRow];
    document.querySelectorAll(".fe-stroke-clips-row").forEach(function (r) {
      allRows.push(r);
    });
  ...
  btnConfirm.addEventListener("click", function () {
    var picked = 0;
    var targetRow = _activeStrokeRow || selectedClipsRow;
    ...
    forEachCard(function (c) {
      ...
      if (targetRow)
        targetRow.appendChild(createClipThumbElement(id, src, isVid));
      if (previewClipsRow)
        previewClipsRow.appendChild(createClipThumbElement(id, src, isVid));
    });
    if (picked === 0) {
      ensureClipRowPlaceholder(targetRow);
      ensureClipRowPlaceholder(previewClipsRow);
    }
    overlay.classList.remove("is-open");
    _activeStrokeRow = null;
  ...
  /* Submit â€” add clip to page + modal list, toast, close */
  ...
  showTab("pro");
  if (overlay) overlay.setAttribute("data-fe-rc-wired", "1");
}

(function bootFeReferenceClips() {
  "use strict";

  var REF_SRC = "../session-evaluation/create.html";
  var MARK_START = "<!-- ====== Reference Clips Modal";
  var MARK_END = "<!-- ====== Success (shared app-modal pattern) ====== -->";

  function injectFragments(html) {
    if (document.getElementById("clipsOverlay")) return true;
    var a = html.indexOf(MARK_START);
    var b = html.indexOf(MARK_END);
    if (a === -1 || b === -1 || b <= a) return false;
    document.body.insertAdjacentHTML("beforeend", html.slice(a, b).trim());
    return true;
  }

  function finish() {
    wireFeReferenceClipsModal();
    if (window.AppShared && window.AppShared.initLucideIcons) {
      window.AppShared.initLucideIcons();
    }
  }

  function boot() {
    if (!document.getElementById("btnRefClips") && !document.querySelector(".fe-stroke-add-clip")) return;

    if (document.getElementById("clipsOverlay")) {
      finish();
      return;
    }

    fetch(new URL(REF_SRC, window.location.href).href)
      .then(function (r) {
        return r.text();
      })
      .then(function (text) {
        if (!injectFragments(text)) return;
        finish();
      })
      .catch(function () {
        if (
          ...
          console.warn &&
          !document.getElementById("clipsOverlay")
        ) {
          console.warn(
            "[fe-reference-clips] Could not load clip modal (embed markup in page or use http://).",
          );
        }
      });
  }
